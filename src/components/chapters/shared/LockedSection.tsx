...
export default function LockedSection({ title, message }: LockedSectionProps) {
  return (
    <div className="p-8 rounded-3xl glassmorphism border border-dark-border bg-dark-bg/80 text-center flex flex-col items-center justify-center min-h-[300px]">
      ...
      <h3 className="font-display font-extrabold text-white text-lg mb-2">{title}</h3>
      <p className="text-slate-500 text-xs sm:text-sm max-w-md">{message}</p>
    </div>
  );
}
