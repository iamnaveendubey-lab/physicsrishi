...
export default function Dashboard() {
  ...
  const totalChapters = chapters.length || 30;
  const completedChaptersList = progress
    .filter((p) => p.chapterCompleted)
    .map((p) => p.chapterId);
  ...
  const activeChapterProgress = progress.find(
    ...
    <div className="relative overflow-hidden bg-dark-bg min-h-[calc(100vh-5rem)] py-12">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <span className="text-xs font-bold text-physics-purple tracking-widest uppercase">
              Student Workspace
            </span>
            ...
            <p className="text-slate-400 text-sm sm:text-base mt-1">
              Here is your physics-specific conceptual roadmap. Track milestones
              ...
              <div className="glassmorphism rounded-3xl p-6 border border-dark-border/40 bg-dark-bg/30 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span
                        className={`font-semibold px-3 py-1 rounded-full text-xs ${
                          user.exam === "jee"
                            ? "bg-jee/10 text-jee border border-jee/20"
                            : "bg-neet/10 text-neet border border-neet/20"
                        }`}
                      >
                ...
                {user.subscription !== "active" && (
                  <button
                    onClick={() =>
                      alert(
                        "Premium memberships are currently in preview mode.",
                      )
                    }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mb-6 leading-relaxed">
                      Syllabus progress index tracks your completed topics and
                      quiz modules across mechanics and electrodynamics.
                    </p>
                    ...
                    <div className="space-y-2 mb-6">
                      ...
                      <p className="text-[10px] text-slate-500">
                        {completedCount} of {totalChapters} chapter modules
                        completed
                      </p>
                    </div>
                  </div>
                ...
                <div className="pt-6 border-t border-dark-border/30 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="text-left">
                    <span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">
                      Current Active Study Target
                    </span>
                    <p className="text-sm font-bold text-white mt-0.5">
                      {currentChapter
                        ? `Chapter ${currentChapter.chapterId}: ${currentChapter.title}`
                        : "Syllabus Completed!"}
                    </p>
                  </div>
                  {currentChapter ? (
                ...
                {currentChapter && (
                  <div className="pt-4 mt-4 border-t border-dark-border/20">
                    <span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold block mb-3">
                      Active Chapter Checklist
                    </span>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {[
                        {
                          name: "Mind Map",
                          completed: activeChapterProgress?.mindMapCompleted,
                        },
                        {
                          name: "Concepts Deck",
                          completed: activeChapterProgress?.conceptsCompleted,
                        },
                        {
                          name: "Formula Sheets",
                          completed:
                            activeChapterProgress?.formulaSheetCompleted,
                        },
                        {
                          name: "Solved Examples",
                          completed: activeChapterProgress?.examplesCompleted,
                        },
                        {
                          name: `Concept Test (${activeChapterProgress?.conceptTestScore || 0}%)`,
                          completed:
                            (activeChapterProgress?.conceptTestScore || 0) >=
                            60,
                        },
                        {
                          name: `Competition Test (${activeChapterProgress?.competitionTestScore || 0}%)`,
                          completed:
                            (activeChapterProgress?.competitionTestScore ||
                              0) >= 70,
                        },
                      ].map((item, i) => (
                        <div
                        ...
                        >
                          {item.completed ? (
                            <svg
                              className="w-3.5 h-3.5 text-emerald-450 shrink-0"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke="currentColor"
                            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
              ...
              <div className="glassmorphism rounded-3xl p-6 border border-dark-border/40 bg-dark-bg/30">
                <h3 className="font-display font-extrabold text-lg text-white mb-4 flex items-center gap-2">
                  <svg
                    className="w-5 h-5 text-emerald-450"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                ...
                {completedTitles.length > 0 ? (
                  <ul className="space-y-2.5">
                    {completedTitles.map((title, idx) => (
                      <li
                        key={idx}
                        className="flex items-center justify-between p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/20 text-emerald-400 text-sm font-semibold"
                      >
                        <span>{title}</span>
                  ...
                  <p className="text-slate-500 text-sm italic py-4">
                    No chapters completed yet. Start with Chapter 1 to unlock
                    progress.
                  </p>
                )}
              </div>
                  ...
                  <div className="p-4 rounded-xl border border-dark-border/30 bg-dark-bg/40 text-sm text-slate-400">
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-bold text-white text-base">
                        Chapter {nextLockedChapter.chapterId}:{" "}
                        {nextLockedChapter.title}
                      </span>
                    ...
                    <p className="text-xs text-slate-500">
                      Unlocks immediately upon finishing the active diagnostic
                      exercises in the previous module.
                    </p>
                  </div>
                ) : (
                  <p className="text-slate-500 text-sm italic py-4">
                    {currentChapter
                      ? "No more locked modules. You are on the final chapter!"
                      : "Syllabus fully unlocked!"}
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-8">
              {[11, 12].map((classLevel) => {
                ...
                return (
                  <div
                    key={classLevel}
                    className="glassmorphism rounded-3xl p-6 sm:p-8 border border-dark-border/40 bg-dark-bg/30"
                  >
                    <h3 className="font-display font-extrabold text-xl text-white mb-6">
                      Class {classLevel} Physics Journey
                    </h3>

                    <div className="space-y-4">
                      {classChapters.map((ch) => {
                        const isCompleted = completedChaptersList.includes(
                          ch.chapterId,
                        );
                          ...
                          >
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-slate-500">
                                  Global module {ch.chapterId} · Class{" "}
                                  {ch.classLevel} Ch {ch.classOrder}
                                </p>
                                {(isActive || isCompleted) && (
                                  <div className="flex flex-wrap gap-2 mt-3 text-[10px]">
                                    {[
                                      {
                                        name: "Mind Map",
                                        completed: chProgress?.mindMapCompleted,
                                      },
                                      {
                                        name: "Concepts",
                                        completed:
                                          chProgress?.conceptsCompleted,
                                      },
                                      {
                                        name: "Formulas",
                                        completed:
                                          chProgress?.formulaSheetCompleted,
                                      },
                                      {
                                        name: "Examples",
                                        completed:
                                          chProgress?.examplesCompleted,
                                      },
                                      {
                                        name: "Concept Test",
                                        completed:
                                          (chProgress?.conceptTestScore || 0) >=
                                          60,
                                      },
                                      {
                                        name: "Comp Test",
                                        completed:
                                          (chProgress?.competitionTestScore ||
                                            0) >= 70,
                                      },
                                    ].map((milestone, idx) => (
                                      <span
